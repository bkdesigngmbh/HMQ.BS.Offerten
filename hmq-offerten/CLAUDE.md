...
5. Handle EMG (`offertart`, modelled after the manually extended Muster-Offerte 51.26.392 "mit EMG"):
   ...
   - `bs_emg`: keep everything; EMG chapter auto-numbers to 3, `{{NR_KOSTEN}}` 4.1, `{{NR_EMGK}}` 4.2, `{{NR_EMG}}` 3.1; page break before the EMG chapter (para with `{{EMG_PB}}`)
   - `emg`: remove BS blocks; subject becomes "Offerte für Erschütterungsmessung" (`{{OFFERT_TITEL}}`), `{{AUSGANGSLAGE_ZIEL}}`/`{{TERMINE_SATZ1}}`/`{{TERMINE_OBJEKT}}` switch to EMG wording, numbers 2.1/3.1, the `{{EMG_PB}}` page-break paragraph, the `{{EMGK_ABSTAND}}` spacer under the KOSTEN heading and the "Installation erfolgt zeitgleich..." bullet are removed; filename prefix "Erschütterungsmessung ¦ "
   - whenever EMG is active, `pageBreakBefore` is injected into the KOSTEN heading (after `keepLines`, schema order) so the chapter title never sits alone at a page bottom
   - all Überschrift-2 titles have spacing before = 0 (template V13, direct formatting + style; feedback BPa 2026-07-29, applies to ALL offer types incl. plain BS)
